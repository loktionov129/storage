...
global $page_title;
$page_title = '';
if (ADDITIONAL_SITE_CODE != $arResult['VARIABLES']['SECTION_CODE_PATH'])
{
	// explode SECTION_CODE_PATH by slash('/')
    $section_path = explode('/', $arResult['VARIABLES']['SECTION_CODE_PATH']);
	$rubric = $section_path[0];
	$gamma = empty($section_path[1]) ? '' : $section_path[1];
	$element = empty($section_path[2]) ? '' : $section_path[2];
	
	// get rubric ID and NAME
	$rub_xml_res = CIBlockPropertyEnum::GetList(array(),array('XML_ID'=>$rubric));
	$rub_xml_id = 0;
	$rub_name = '';
	if($prop = $rub_xml_res->Fetch()){
		$rub_xml_id = intval($prop['ID']);
		$rub_name = $prop['VALUE'];
	}
	$APPLICATION->AddChainItem($rub_name, "/catalog/" . $rubric . '/');
	$page_title = $rub_name;
	$myFilter = array("RUBRIC"=>$rub_xml_id);
	
	// get gamma ID and NAME
	if ($gamma)
	{
	    $gamma_xml_res = CIBlockPropertyEnum::GetList(array(),array('XML_ID'=>$gamma));
	    $gamma_xml_id = 0;
	    if($prop = $gamma_xml_res->Fetch()){
		    $gamma_xml_id = intval($prop['ID']);
			$APPLICATION->AddChainItem($prop['VALUE'], "/catalog/" . $rubric . '/' . $gamma . '/');
			$page_title = $prop['VALUE'];
	    }
	    $myFilter["GAMMA2"] = $gamma_xml_id;
	}
	
	$arrFilter['PROPERTY'] = $myFilter;
}
...
<?	
    if ($element):
		$intElementID = $APPLICATION->IncludeComponent(
			...
			array(
				...
				"ELEMENT_ID" => 0, //$arResult["VARIABLES"]["ELEMENT_ID"],
				"ELEMENT_CODE" => $element, //$arResult["VARIABLES"]["ELEMENT_CODE"],
			...
			$intSectionID = $APPLICATION->IncludeComponent(
				...
				array(
					...
					"SECTION_ID" => "", //$arResult["VARIABLES"]["SECTION_ID"],
					"SECTION_CODE" => ADDITIONAL_SITE_CODE, //$arResult["VARIABLES"]["SECTION_CODE"],
					"SECTION_URL" => $arResult["FOLDER"].$arResult["URL_TEMPLATES"]["section"],
					"DETAIL_URL" => $arResult["FOLDER"].$arResult["URL_TEMPLATES"]["element"],
					"USE_MAIN_ELEMENT_SECTION" => $arParams["USE_MAIN_ELEMENT_SECTION"],
					'CONVERT_CURRENCY' => $arParams['CONVERT_CURRENCY'],
