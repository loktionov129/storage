$('.MYTABLE thead').on('click', 'td', function() {
	...
	function compareTR(a, b) {
		...
		if (a.find('td').eq(indexCell).find('a').length > 0) {
			newA = a.find('td').eq(indexCell).find('a').html().replace(/ /g, "");
			if (b.find('td').eq(indexCell).find('a').length > 0)
				newB = b.find('td').eq(indexCell).find('a').html().replace(/ /g, "");
		} else {
			newA = a.find('td').eq(indexCell).html().replace(/ /g, "");
	...
	$(this).closest('tr').find('th').removeClass('adm-list-table-cell-sort-down adm-list-table-cell-sort-up').addClass('adm-list-table-cell-sort');
